...
path_deliver_tiff = sprintf('%s%s_%s_%s%s',path_target(1:slashix(end-1)),'Deliver',info_initials, tstr,info_slash);
command = sprintf('mkdir %s', path_deliver_tiff);
[~,~] = system(command);
...
for d = 1:n_D;
    ...
    path_tiff = sprintf('%s%s+tiff%s',path_sub,D{d},info_slash);
    ...
    %}
     ...
     path_sub_deliver_tiff = sprintf('%s%s%s', path_deliver_tiff, D{d}, info_slash);
    command = sprintf('mkdir %s', path_sub_deliver_tiff);
    [~,~] = system(command); 
    path_sub_deliver_tiff = sprintf('%s%s%s%s+tiff%s', path_deliver_tiff, D{d}, info_slash, D{d}, info_slash);
    command = sprintf('mkdir %s', path_sub_deliver_tiff);
    [~,~] = system(command); 
...
%     [~,~] = system(command);
    %filepath_cubelist = sprintf('%s%s_cube.txt',path_matlab,D{d});
    ...
    for b = 1:n_bands;
         filepath_source = sprintf('%s%s+%s_stretch.tif', path_tiff, D{d},all_bands{b});
         filepath_target = sprintf('%s%s+%s_stretch.tif', path_sub_deliver_tiff, D{d},all_bands{b});
          if ispc();

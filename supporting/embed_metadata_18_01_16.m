function [  ] = embed_metadata_18_01_16(  )
...
for s = 1:n_source;
    ...
    if ~is_cubelist
        fprintf('\nCube list does not exist.Choosing source images manually\n');
        % Determine directory of tiffs 
        cd(path_source_previous);
        titlestr = sprintf('Please choose parent files for %s', SourceFile');
        [Flattened_Files, ~] = uigetfile('*.tif',titlestr, 'MultiSelect', 'on');
        Flattened_Files = Flattened_Files';
        n_Flattened_Files = numel(Flattened_Files);
        wavelength_all = cell(n_Flattened_Files,1);
        for ff = 1:n_Flattened_Files;
            ix_delim1 = strfind(Flattened_Files{ff},options_delimiter_wavelength);
            ix_delim2 = strfind(Flattened_Files{ff},options_delimiter);
            wavelength_all{ff} = Flattened_Files{ff}(ix_delim1:ix_delim2(end-1)-1);
        end
        name = Flattened_Files{ff}(1:ix_delim1-1);
    end
    ...
    elseif strcmp(type, 'other');
        titlestr = sprintf('Please type description of processing and click enter (e.g., RGB image from PCA bands with band ratio.) \n %s',SourceFile);
        info.DAT_File_Processing =  inputdlg(titlestr);
        titlestr = sprintf('Please type short processing tag and click enter (e.g., txratio, PCA, sharpie.) \n %s',SourceFile);
        processing_type =  inputdlg(titlestr);
        processing_type = char(processing_type{1});
        processing_type = strrep(processing_type, ' ', '');
...
parentpath = default.parentpath;% Roger
    ...
    [~,~] = system(sprintf('%s %s',info_rmcall, jsonfilename));

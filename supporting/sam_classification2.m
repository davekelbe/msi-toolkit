function [ output_args ] = sam_classification2( input_args )
...
mask = imread('/Users/Kelbe/Desktop/stick_mask2.tif');
mask = double(mask);
mask = mask./max(mask(:));
mask = logical(mask);
% figure; imagesc(mask);

path_ref = '/Volumes/Seagate Backup Plus Drive/CAPTURE_SpectraHutt/FLATTENED/Assorted_Papyrus/GC_PAP_000389_bib/';
D = dir(path_ref);
...
mask_spectralon = imread('/Users/Kelbe/Desktop/MOTB/Processed/GC_PAP_000389_bib/GC_PAP_000389_bib+tiff/GC_PAP_000389_bib_spectralon_mask.tif');
% Load each image, normalize to spectralon, and save average value of mask
path_save = '/Users/Kelbe/Desktop/MOTB/Processed/GC_PAP_000389_bib/GC_PAP_000389_bib+norm/';
% for w = 1:n_w
...
I = imread(sprintf('%s%s', path_ref, 'GC_PAP_000389_bib+MB365UV_011_F.tif'));
maxVal = median(I(mask_spectralon));
I = double(I)./double(maxVal);
chopsticks_spectra(1) = median(I(mask));
I = imread(sprintf('%s%s', path_ref, 'GC_PAP_000389_bib+MB530GN_005_F.tif'));
maxVal = median(I(mask_spectralon));
I = double(I)./double(maxVal);
chopsticks_spectra(2) = median(I(mask));
I = imread(sprintf('%s%s', path_ref, 'GC_PAP_000389_bib+MB850IR_012_F.tif'));
maxVal = median(I(mask_spectralon));
I = double(I)./double(maxVal);
chopsticks_spectra(3) = median(I(mask));
I = imread(sprintf('%s%s', path_ref, 'GC_PAP_000389_bib+W365B47_021_F.tif'));
maxVal = median(I(mask_spectralon));
I = double(I)./double(maxVal);
chopsticks_spectra(4) = median(I(mask));
I = imread(sprintf('%s%s', path_ref, 'GC_PAP_000389_bib+W365O22_014_F.tif'));
maxVal = median(I(mask_spectralon));
I = double(I)./double(maxVal);
